...
-- Rename the existing business_category column temporarily
ALTER TABLE public.provider_details 
RENAME COLUMN business_category TO business_category_old;

-- Add new business_category column as UUID
ALTER TABLE public.provider_details 
ADD COLUMN business_category UUID;
...
-- Update existing records to use 'Other' category if they had text data
UPDATE public.provider_details 
SET business_category = (SELECT id FROM public.business_categories WHERE category_type = 'other' LIMIT 1)
WHERE business_category_old IS NOT NULL;

-- Drop the old column
ALTER TABLE public.provider_details 
DROP COLUMN business_category_old;