...
-- Update provider_details to use proper business_category reference
ALTER TABLE public.provider_details 
ALTER COLUMN business_category TYPE UUID USING (
  CASE 
    WHEN business_category IS NULL THEN NULL
    ELSE (SELECT id FROM public.business_categories WHERE category_type = 'other' LIMIT 1)
  END
);
