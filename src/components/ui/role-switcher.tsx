...
export const AddRoleCard = () => {
  const { availableRoles, addRole } = useAuth();
  const [businessName, setBusinessName] = useState('');
  const [isLoading, setIsLoading] = useState(false);

  const missingRole = availableRoles.includes('customer') ? 'provider' : 'customer';
  
  if (availableRoles.length === 2) {
    return null; // User has both roles
  }

  const handleAddRole = async () => {
    if (missingRole === 'provider' && !businessName.trim()) {
      toast.error('Business name is required for provider role');
      return;
    }

    setIsLoading(true);
    const { error } = await addRole(missingRole, businessName.trim() || undefined);
    setIsLoading(false);
    
    if (error) {
      toast.error('Failed to add role');
    } else {
      toast.success(`${missingRole} role added successfully`);
      setBusinessName('');
    }
  };
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          Add {missingRole === 'customer' ? 'Customer' : 'Provider'} Role
        </CardTitle>
        <CardDescription>
          {missingRole === 'customer' 
            ? 'Book services and manage your appointments' 
            : 'Start offering services and grow your business'
          }
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <Button onClick={handleAddRole} disabled={isLoading} className="w-full">
          {isLoading ? 'Adding...' : `Add ${missingRole} Role`}
        </Button>
      </CardContent>
    </Card>
  );
